function write_nc(data,gridded,cfg,filename,time_range)
    ...
    % get time index 
    gridded_time = datetime(gridded.dn,'ConvertFrom','datenum');
    time_idx = (gridded_time >= time_range(1)) & (gridded_time <= time_range(2));
    ...
    function var_id = def_info_time_variable(varname)
        var_id = [];
        if isnonemptyfield(gridded.info,varname)
    ...
    if isnonemptyfield(cfg,'raw2mat')
    ...
    netcdf.putVar(grid_id,time_id,posixtime(gridded_time(time_idx)));
    netcdf.putVar(grid_id,dn_id,gridded.dn(time_idx));
    ...
    for ii = 1:length(data)
        ...
        % get time index
        sensor_time = datetime(data{ii}.dn,'ConvertFrom','datenum');
        time_idx = (sensor_time >= time_range(1)) & (sensor_time <= time_range(2));
        
        % total number of datapoints
        ndata = length(data{ii}.dn);
        nidx = nnz(time_idx);
        ...
        netcdf.putVar(grp_id,time_id,posixtime(sensor_time(time_idx)));
