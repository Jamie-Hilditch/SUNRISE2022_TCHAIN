...
signal1=medfilt1(signal1,10,'omitnan','truncate');
signal2=medfilt1(signal2,10,'omitnan','truncate');
...
p=fast_cohere(signal1(tinds1(1:leni)),signal2(tinds2(1:leni)),256,FS); %256 points
...
fig = figure;
subplot(311)
plot(datetime(time1(tinds1(1:leni)),'ConvertFrom','datenum'),signal1(tinds1(1:leni)),...
  datetime(time2(tinds2(1:leni)),'ConvertFrom','datenum'),signal2(tinds2(1:leni)))
legend('Signal 1','Signal 2')
subplot(312)
plot(p.f,p.coh)
title('Coherence')
subplot(313)
plot(p.f,unwrap(p.pha),p.f(ginds),p.f(ginds)*b(1))
