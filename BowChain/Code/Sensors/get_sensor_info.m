...
% SBE39
serials.sbe39 = readmatrix('sbe39.dat');
fmt.sbe39 = '%05s';

% SBE56
serials.sb356 = readmatrix('sb356.dat');
fmt.sbe56 = '%05s';

% RBR Solo
serials.rbr_solo = readmatrix('rbr_solo.dat');
fmt.rbr_solo = '%06s';

% Duet
serials.rbr_duet = readmatrix('rbr_duet.dat');
fmt.rbr_duet = '%06s';

% RBR Concerto
serials.rbr_concerto = readmatrix('rbr_concerto.dat');
fmt.rbr_concerto = '%06s';

% GusT
serials.gust = readcell('gust.dat');
fmt.gust = '%s';
...
%                     101195, 101196, 101197, 102516, 102517, 102519, 101187, ...
...
%                     207024, 207060, 207023, 207030, 207027, 207028, 207018, ...
%                     207053, 207051, 100033, 100032, 207055, 102518];

% serials.rbr_duet = [082489, 082506, 207010, 203187, 203188, 203186, 203184, 203185, ...
%                     207015, 207013, 207012, 207014, 082492, 207007, 207006, 207009, ...
%                     207008];

% serials.rbr_concerto = [060280, 060281, 060088, 060094, 060095, 060559, 060704, ...
%                         060276, 060275, 060381, 060093, 060380, 060528, 060702, ...
%                         060703, 060379, 060701, 060183, 060558, 060559];
